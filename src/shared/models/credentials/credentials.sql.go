...
const getUserWithCredentialsByID = `-- name: GetUserWithCredentialsByID :many
SELECT webauthn_users._id, webauthn_users.ref_id, webauthn_users.raw_id, webauthn_users.name, webauthn_users.display_name, webauthn_credentials.credential_id, webauthn_credentials.user_id, webauthn_credentials.use_counter, webauthn_credentials.public_key, webauthn_credentials.attestation_type, webauthn_credentials.transport, webauthn_credentials.flags, webauthn_credentials.authenticator, webauthn_credentials.attestation
FROM webauthn_users
...
func (q *Queries) GetUserWithCredentialsByID(ctx context.Context, ID int64) ([]GetUserWithCredentialsByIDRow, error) {
	rows, err := q.db.Query(ctx, getUserWithCredentialsByID, ID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetUserWithCredentialsByIDRow
	for rows.Next() {
		var i GetUserWithCredentialsByIDRow
		if err := rows.Scan(
			&i.WebauthnUser.ID,
			&i.WebauthnUser.RefID,
			&i.WebauthnUser.RawID,
...
const getUserWithCredentialsByRef = `-- name: GetUserWithCredentialsByRef :many
SELECT webauthn_users._id, webauthn_users.ref_id, webauthn_users.raw_id, webauthn_users.name, webauthn_users.display_name, webauthn_credentials.credential_id, webauthn_credentials.user_id, webauthn_credentials.use_counter, webauthn_credentials.public_key, webauthn_credentials.attestation_type, webauthn_credentials.transport, webauthn_credentials.flags, webauthn_credentials.authenticator, webauthn_credentials.attestation
FROM webauthn_users
LEFT JOIN webauthn_credentials ON webauthn_users._id = webauthn_credentials.user_id
...
func (q *Queries) GetUserWithCredentialsByRef(ctx context.Context, refID string) ([]GetUserWithCredentialsByRefRow, error) {
	rows, err := q.db.Query(ctx, getUserWithCredentialsByRef, refID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetUserWithCredentialsByRefRow
	for rows.Next() {
		var i GetUserWithCredentialsByRefRow
		if err := rows.Scan(
			&i.WebauthnUser.ID,
			&i.WebauthnUser.RefID,
			&i.WebauthnUser.RawID,
...
func (q *Queries) ListCredentialsByUser(ctx context.Context, userID pgtype.Int8) ([]ListCredentialsByUserRow, error) {
	rows, err := q.db.Query(ctx, listCredentialsByUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListCredentialsByUserRow
	for rows.Next() {
